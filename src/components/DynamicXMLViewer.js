...
export function DynamicXMLViewer({onSelection, setSelection, currentPage, setAnnoZones}) {
    ...
    const customPlugin = (node, children, attributes) => {

        if (node.tagName.toLowerCase() === "line") {
            attributes.facs = '#' + node.parentNode.getAttribute('xml:id')
        ...
        <Fragment>
            <div className="h-100 d-flex flex-column">
                ...
                <div className={"xml-container"} >
                    {showRender ? (
                        <SelectableXmlViewer ref={containerRef} xmlString={xmlText} onSelection={onSelection} setSelection={setSelection} onZonesExtracted={setAnnoZones} customRenderers={customRenderers} plugins={[customPlugin]} />
                    ) : (
